...
from telegramusers import models
...
class TestTelegramUserModel:
    ...
    def get_single_model(cls, django_db_setup, django_db_blocker):
        with django_db_blocker.unblock():
            ...
            model = models.TelegramUserModel.objects.create(telegram_user_id=12345, user=user)
            model.full_clean()
            return model
    ...
    @pytest.mark.parametrize(
        ...
        [
            (models.TelegramUserModel.user, 'Пользователь'),
            (models.TelegramUserModel.telegram_user_id, 'Идентификатор пользователя в Telegram'),
            (models.TelegramUserModel.created_at, 'Дата создания'),
            (models.TelegramUserModel.updated_at, 'Дата обновления'),
        ],
    )
    ...
    def test_user_one_to_one_primary_key(self):
        user = User.objects.create_user(username='username', password='password', email='example@domain.ru')
        models.TelegramUserModel.objects.create(telegram_user_id=1, user=user)
        with pytest.raises(IntegrityError):
            models.TelegramUserModel.objects.create(telegram_user_id=2, user=user)
    ...
    def test_create_exists_telegram_user_id(self):
        ...
        models.TelegramUserModel.objects.create(telegram_user_id=1, user=user1)
        with pytest.raises(IntegrityError):
            models.TelegramUserModel.objects.create(telegram_user_id=1, user=user2)

    @pytest.mark.django_db
    def test_null_parameters(self):
        with pytest.raises(IntegrityError):
            models.TelegramUserModel.objects.create(telegram_user_id=None)

    @pytest.mark.django_db
    def test_count_elements_in_table(self, get_single_model):
        assert models.TelegramUserModel.objects.count() == 1
    ...
    def test_get_user(self, tgbot_update_and_context_mock):
        ...
        models.TelegramUserModel.objects.create(telegram_user_id=1, user=user)
        user = models.TelegramUserModel.get_user(update, context)
        assert isinstance(user, models.TelegramUserModel)
